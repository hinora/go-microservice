...
b.LoadService(&Service{Name: "math", Actions: []Action{{Name: "add", Handle: func(ctx *Context) (interface{}, error) {
p := ctx.Params.(map[string]interface{})
return p["a"].(int) + p["b"].(int), nil
}}}})
waitUntil(t, func() bool { return expvar.Get(MCountCall+".node-action.math.add") != nil })
...
After:  []AfterHookFunc{func(ctx *Context, result interface{}) (interface{}, error) { order = append(order, "service-after"); return result.(string) + "!", nil }},
},
Actions: []Action{{
Name:   "hello",
...
waitUntil(t, func() bool { return expvar.Get(MCountCall+".node-cache.greeter.hello") != nil })
...
b.LoadService(&Service{Name: "listener", Events: []Event{{Name: "order.*", Handle: func(ctx *Context) {
got <- ctx.Params
}}}})
waitUntil(t, func() bool { return len(b.registryServices) == 1 })
...
b.LoadService(&Service{Name: "slow", Actions: []Action{{Name: "wait", Timeout: 5, Handle: func(ctx *Context) (interface{}, error) {
atomic.AddInt32(&slowCalls, 1)
time.Sleep(20 * time.Millisecond)
return "late", nil
}}}})
waitUntil(t, func() bool { return expvar.Get(MCountCall+".node-resilience.slow.wait") != nil })
...
b2.LoadService(&Service{Name: "limited", Actions: []Action{{Name: "run", Bulkhead: &BulkheadConfig{MaxConcurrency: 1}, Handle: func(ctx *Context) (interface{}, error) {
<-block
return "ok", nil
}}}})
waitUntil(t, func() bool { return expvar.Get(MCountCall+".node-bulkhead.limited.run") != nil })
...
for _, tc := range []struct{ pattern, name string; want bool }{
...
lc.logInfo(lc.data[0]); lc.logWarning(LogData{Type: LogTypeWarning, Time: 1, Message: "warn"}); lc.logError(LogData{Type: LogTypeError, Time: 1, Message: "err"})
...
b := &Broker{Config: BrokerConfig{NodeId: "gw-node"}, bus: initEventBus(), logs: Log{}, traceSpans: map[string]*traceSpan{}}
b.trace = Trace{}
gatewayService := InitGateway(GatewayConfig{Name: "api"})
b.LoadService(gatewayService)
waitUntil(t, func() bool { return len(b.registryServices) == 1 })
